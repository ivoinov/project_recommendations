...
def test_token_user_id_column():
    ...
    assert isinstance(column.type, Integer), "'user_id' column should be of type Integer"
    assert column.foreign_keys is not None, "'user_id' column should have a foreign key"

def test_token_user_relationship():
    assert 'user' in Token.__mapper__.relationships, "Token model should have a 'user' relationship"
...
def test_token_expires_at_column():
    ...
    assert isinstance(column.type, DateTime), "'expires_at' column should be of type DateTime"